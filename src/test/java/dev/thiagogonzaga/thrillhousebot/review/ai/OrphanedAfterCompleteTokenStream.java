...
/** TokenStream that completes on a background thread after a delay. */
final class OrphanedAfterCompleteTokenStream implements TokenStream {

  private final long completeDelayMs;
  private Consumer<ChatResponse> completeHandler;

  OrphanedAfterCompleteTokenStream(long completeDelayMs) {
  ...
  public void start() {
    var completer =
        new Thread(
            () -> {
              if (!TestDelays.simulateLatency(completeDelayMs)) {
                return;
              }
              if (completeHandler != null) {
                completeHandler.accept(
                    ChatResponse.builder().aiMessage(AiMessage.from("{\"findings\":[]}")).build());
              }
            },
            "orphaned-after-complete-token-stream");
