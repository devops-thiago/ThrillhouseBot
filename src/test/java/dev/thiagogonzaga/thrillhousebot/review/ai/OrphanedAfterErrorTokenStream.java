...
/**
 * TokenStream that fails immediately, then keeps emitting partial tokens on a background thread.
 */
final class OrphanedAfterErrorTokenStream implements TokenStream {

  private final long tokenDelayMs;
  private Consumer<String> partialHandler;
  private Consumer<Throwable> errorHandler;

  OrphanedAfterErrorTokenStream(long tokenDelayMs) {
  ...
  public void start() {
    if (errorHandler != null) {
      errorHandler.accept(new RuntimeException("stream failed"));
    }
    var emitter =
        new Thread(
            () -> {
              for (var i = 0; i < 20; i++) {
                ...
                if (partialHandler != null) {
                  partialHandler.accept("late-" + i);
                }
              }
            },
            "orphaned-after-error-token-stream");
