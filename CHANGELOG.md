...
- **CI status resolves concurrently with the model call, with one fewer API request**: the required-check resolution and CI evaluation depend only on the commit and base branch, not the model response, so they now run on the review executor concurrently with the (blocking) AI call instead of strictly after it — the GitHub latency overlaps the model latency rather than stacking on top of it. The base branch is now carried on the review request (from the webhook payload or the manual-trigger PR fetch), so the CI resolver no longer issues a separate `getPullRequest` just to read the target branch. Pagination for check runs and combined status moved into `GitHubCheckRunClient` (`getAllCheckRuns`/`getAllCombinedStatus`), matching the other GitHub clients (no behavior change) (#250)
- **Review context loads the ignore-glob filter once**: the changed-file list was run through the ignore-glob filter twice per review — `reviewableFiles` was computed and then `patchesByFile` recomputed it internally. The already-filtered list now feeds the line resolver directly, halving the glob matching on the hot review path for large PRs (no behavior change) (#250)
...
- **No "completed" event when the completion write fails**: the post-result steps were split so a failure in one couldn't abort the rest, but "mark the session completed" and "broadcast completion" became separate steps — `applyReviewResult` flips the in-memory session to completed and *then* persists, so if the persistence write threw, the swallowed failure still let the next step broadcast a `completed` event. The live dashboard showed the session completed while its row stayed in progress (reverting on reload). The persist and the broadcast are now one step: a failed write skips the broadcast, so persisted and live state stay consistent (#254)
- **Unreadable CI is disclosed even when a check is also failing**: the no-issues review body chained the offending-checks and unreadable-CI notes with `else if`, so a review held by both at once only mentioned the failing check and dropped the unreadable note — disagreeing with the summary table, which lists them independently. The two are now disclosed independently in the body as well (#253)
- **CI bot-check detection follows the configured identity**: the CI gate recognizes the bot's own checks from the configured bot logins (the shared identity, which covers both the default and the alternate `<slug>[bot]`) instead of a single hardcoded name token, so a deployment under the alternate slug can't have its own app's checks counted as offending CI (#253)
- **Post-result steps no longer leave a session stuck in progress**: once the review and its comments are on the PR, the remaining work (concluding the check run, resolving addressed threads, applying labels, marking the session completed, broadcasting completion) now runs as independent best-effort steps. Previously a failure in any of them — most importantly the session-completion persistence write — jumped to a catch that only logged, skipping the completion broadcast and leaving the session shown as perpetually running in the dashboard even though the review had posted. Each step is now isolated, so a hiccup in one can't abort the others, and the session always reaches a terminal state (#254)
- **Walkthrough-diagram kill switch is now honored at render**: the `enabled` master switch gated only the prompt request, not the render, so a model that volunteered a `walkthrough_diagram` field the prompt never asked for could still produce a Control-Flow Diagram block while the feature was disabled. The summary now also drops the diagram at render time when the switch is off, matching the documented "no diagram is requested or rendered unless enabled" (#181)
...
- **Unreadable CI status no longer allows an approval**: the CI gate works off the *offending* checks, and an empty list meant "nothing blocks". But when the Check Runs / Combined Status API threw or returned a null body, that empty list was indistinguishable from "CI all green", so a PR whose CI was actually failing or still pending could receive an APPROVE on a transient GitHub hiccup. The evaluator now reports an unread CI source as a first-class signal rather than a synthetic "CI status unavailable" check: it holds the verdict to a comment (findings still post, only the approval is held) and is disclosed as its own "CI Status Unavailable" note in the summary instead of appearing as a counterfeit required check. The hold now applies in **both** gate modes — gate-specific is not automatically safe, since an unread source can hide a required check's true state — closing the gap where a gate-specific review could still approve over CI it never read (#253)
