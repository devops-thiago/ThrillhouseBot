...
- **Config-key definition files under an ignored path are no longer fetched or rendered** (#483): the definition sites the review pulls in for a documented config key come from a walk of the whole repository tree, and that walk was never handed the review's ignore globs — only the changed files that name the keys were filtered. A repository that excluded `config/` from review therefore still had `config/application.yml` read and quoted into the prompt whenever a doc line mentioned a key it defines. The candidate files now pass through the same compiled ignore set, with the same gitignore reading, as the diff itself, so a path the repository asked the review not to look at is left alone on both sides
