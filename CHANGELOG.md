...
- **Patch coverage in the review context** (#467): when a PR's CI publishes a coverage report, the changed lines it does not cover are given to the review, so untested new code can be named as such. Enable with `REVIEW_PATCH_COVERAGE_ENABLED`
...
- **Follow-up delta comment** (#459): a follow-up review can post a short summary of what changed since the last one, listing new, resolved and still-open findings. Enable with `REVIEW_FOLLOW_UP_SUMMARY_ENABLED`
...
- **Large-PR notice** (#466): a large PR whose review produced no inline finding now says so explicitly
...
- **`/describe`, `/changelog`, `/add-docs` and `/generate-tests` are planned as token-budgeted batches** (#463, #469): these commands no longer render one call capped by `REVIEW_MAX_DIFF_LINES`, so a large PR is covered whole. `REVIEW_MAX_DIFF_LINES` now applies only to the remaining single-call renders, which are replies, base comparison, and review with budgeting disabled
