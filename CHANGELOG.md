...
- **Hosted documentation site**: the docs are now published to GitHub Pages at <https://devops-thiago.github.io/ThrillhouseBot/> — getting started, command list, configuration reference, provider table, architecture, comparison, and contributing guide, built with Astro Starlight from `website/` and linked from the README. Repo files (README sections, `CONTRIBUTING.md`, `docs/*.md`) stay the single source of truth via build-time includes. The site also hosts `install.html`, which builds the GitHub App manifest from a typed-in hostname (Smee-aware), so the first App registration no longer needs a local static server or `gh api`. Docs builds fail CI on broken internal links. The configuration reference and architecture page now disclose the **AI call budget**: with `REVIEW_VERIFIER_ENABLED` (default on), a review that reports findings makes two model calls — review + finding verification — and large PRs under token budgeting make N+N+1; the flag, its fail-open behavior, and the cost trade-off are documented in the README config table and `.env.example` (#319)
- **👀 ack reaction on commands**: the bot now reacts 👀 to the comment that triggered any slash or `@mention` command (`/review`, `/help`, `/describe`, …) the moment the webhook is received, so the commenter gets an immediate "seen it" signal while the actual work — which can take minutes — runs in the background. The reaction fires before the pause/authorization gates (a rejected command was still *seen*) and is best-effort: a failed reaction is logged and never blocks the command, and the wait on the ack thread is bounded by `ACK_REACTION_TIMEOUT` (default 3s) so a degraded GitHub can never delay the webhook `200` past the delivery SLA. Conversational `@thrillhousebot` mentions (no command word) are answered, not reacted to — including inline review-thread mentions (#315)
...
- **Per-model AI configuration**: model settings can now be set **per model** under `thrillhousebot.ai.models.<model>.*`, keyed by the `AI_MODEL` name like the pricing map — keep entries for every model you use and switch `AI_MODEL` freely. `max-input-tokens` is the model's input hard cap: the review token budget becomes `min(REVIEW_MAX_INPUT_TOKENS, cap)` with a 128 000 default cap for models without an entry, so a raised budget can never overshoot an unknown model's context window (startup warns whenever the cap lowers the configured budget, and an explicit `REVIEW_MAX_INPUT_TOKENS=0` still disables budgeting). `output-buffer-tokens` and `token-safety-margin` gain per-model overrides, and the generation parameters `temperature`, `top-p`, and `max-output-tokens` are sent on every chat call — including the streaming review call — when set (`top_k` does not exist on the OpenAI-compatible wire). Every entry is range-validated at boot, inactive models' included, so a typo fails startup with a message naming the key. The README configuration table now documents every review tuning knob (`thrillhousebot.review.*`, with the Quarkus env-var mapping and `ignored-files` guidance) plus the new per-model surface, and `.env.example` surfaces the common ones (#50)
- **Reasoning-effort control**: reasoning-capable models (OpenAI o-series/gpt-5.x, Gemini 2.5, qwen3, ...) previously ran with whatever reasoning the provider defaulted to — uncontrollable and silently reflected in cost and output length. `AI_REASONING_ENABLED` (default `false`) plus `AI_REASONING_EFFORT` (`none`/`low`/`medium`/`high`, default `low`) now send the OpenAI-compatible `reasoning_effort` on every AI call — including the streaming review call — giving the operator the cost/quality dial; `none` explicitly asks a model not to reason, and when disabled no parameter is sent so provider defaults keep applying. Invalid efforts are rejected at boot (#65)
