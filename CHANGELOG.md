...
- **A review response that splits its JSON across two documents is read whole, and one that was not fully read is not approved** (#805): a reasoning model answered with prose, then an object carrying the summary, then a fenced second object carrying the findings. Jackson's `readTree` returns after the first complete value and ignores whatever follows, so the second document was dropped without a log line, the root had no `findings` node, that read as an empty list, and the pull request was approved with 0 findings — the finding dropped in the observed run was a public-endpoint 500 on a registration path. The parser now reads every JSON object in the response and merges them: findings append in document order, an identical finding is kept once, and any other field keeps the first non-null value it was given; one warning records how many documents and characters lay past the first. A closing sentence after the last document is discarded with a warning that says how much was dropped, while a further document that is cut or malformed fails the response as malformed, so it is retried rather than approved from a partial read. A root with no `findings` node at all is a format failure for the same reason — only an explicit `"findings": []`, which both prompts ask for, is a clean review
