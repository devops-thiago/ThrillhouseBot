...
- **Hosted documentation site**: the docs are now published to GitHub Pages at <https://devops-thiago.github.io/ThrillhouseBot/> — getting started, command list, configuration reference, provider table, architecture, comparison, and contributing guide, rendered from `docs/` with MkDocs Material and linked from the README. The site also hosts `install.html`, which builds the GitHub App manifest from a typed-in hostname, so the first App registration no longer needs a local static server or `gh api`. Docs builds run `mkdocs build --strict` on pull requests, so a broken internal link fails CI (#319)
