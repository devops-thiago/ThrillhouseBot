...
- **The clear directive now answers to the configured bot login, not a hardcoded slug** (#679): the `resolved` directive pattern is built from `github.bot-logins` (each name regex-quoted, compiled once per configuration), and the conversational-mention gate that fronts the directive's acknowledgements answers to the same configured names, so an install whose GitHub App runs under a different login can clear findings from the PR conversation. The no-locator and no-write-access acknowledgements and the cleared-finding note spell the directive with that install's own mention name; default-config behavior, case-insensitivity and the interrogative (`resolved?`) guard are unchanged
...
- **A backticked quotation in a decline no longer reopens it** (#652): inline code spans are stripped from a maintainer's reply before the decline re-check, so quoting a construct such as `` `executor.submit` `` is not matched as an assertion about the reviewed code
- **A Unicode format character no longer defeats the whole-locator guards** (#654): a zero-width space, zero-width joiner, soft hyphen or BOM inside a range spelling (`src/A.java:1<U+200B>-3`) read `:1` as a whole locator and over-cleared the distinct finding at line 1. Format characters now continue the line-number token, so the locator is not whole and the finding is held
- **A digit-leading finding title after the separator now clears the finding it names** (#653): `@thrillhousebot resolved src/A.java:1 — 2 call sites of …` — the exact form the summary prints — read as a line range, so the finding stayed held and the maintainer was told the comment named no finding. The clearing path now recognizes the summary's own printed row — the em dash followed by the finding's full title — and clears the finding it names, while any other spaced separator keeps reading as a range; the acknowledgement for the ambiguous shape names the ambiguity instead of denying the naming
