...
- **No crash on a prior review from a deleted account**: the first-visible-review check dereferenced each prior review's author with no null guard, so a review left by a since-deleted GitHub account (serialized as `user: null`) threw a NullPointerException and failed the whole review. The author is now null-checked, matching the sibling comment and summary checks
