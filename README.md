...
| `AI_REASONING_EFFORT` | Effort sent while enabled: `none`/`low`/`medium`/`high`/`xhigh`/`max` (`none` explicitly asks the model not to reason; `xhigh`/`max` are the extended tiers newer reasoning models expose above `high`); reasoning tokens are billed as output tokens. A streamed review or summary call that spends its whole output cap reasoning and returns no answer is repeated once with reasoning off, and the review's scope note says so | `low` |
...
| `THRILLHOUSEBOT_REVIEW_MAX_AI_RETRIES` | Attempts per failed AI call before the review errors out. At most two of them may end at `THRILLHOUSEBOT_REVIEW_AI_TIMEOUT_SECONDS`; the rest of the budget is for the other transient failures | `5` |
| `THRILLHOUSEBOT_REVIEW_AI_RETRY_BASE_DELAY_MS` | Base delay of the exponential retry backoff, in milliseconds | `2000` |
| `THRILLHOUSEBOT_REVIEW_AI_TIMEOUT_SECONDS` | Client-side wait per AI streaming attempt; keep it >= `AI_TIMEOUT` so timed-out attempts don't leave orphaned provider streams. One call may spend it twice at most: a second attempt that also times out fails the call instead of waiting out the remaining retries | `300` |
...
- **You name the finding by *both* its `path:line` locator and its own content**
  ...
  one of the two clears nothing. Locators match whole, with the path written from the
  repository root as the row prints it, so `Widget.java:42` never
...
neutral check run instead of an `APPROVE`. CI is read while the model call runs, and
when that reading would hold approval it is read again after the call, so a check
that finished in the meantime counts. That verdict is held, not final: the bot
